...
EddyProc.C <-sEddyProc$new('CH-CHA',  EddyData.F,
                           c('NEE_CUT_16','Rg','Tair','VPD'), ColPOSIXTime = "TIMESTAMP")   
...
write.csv(FilledEddyData.F, file = paste("L:/Sync/luhk_work/20 - CODING/29 - WORKBENCH/dataset_cha_fp2024_2005-2023/notebooks/33_FLUXES_L4_REddyProc_GapFilling_Partitioning_IRGA/33.4_FLUXES_L4_MDSgapfilledPart_NEECUT16_", run_id,".csv",sep="")) 


# PLOT: FINGERPRINTS
# ==================
# Save fingerprint plots as PDF
outdir = "L:/Sync/luhk_work/20 - CODING/29 - WORKBENCH/dataset_cha_fp2024_2005-2023/notebooks/33_FLUXES_L4_REddyProc_GapFilling_Partitioning_IRGA/33.4_plots_NEECUT16/"
EddyProc.C$sPlotFingerprint('NEE_CUT_16_f', Dir = outdir)
EddyProc.C$sPlotFingerprint('GPP_CUT_16_f', Dir = outdir)
EddyProc.C$sPlotFingerprint('GPP_DT_CUT_16', Dir = outdir)
EddyProc.C$sPlotFingerprint('Reco_CUT_16', Dir = outdir)
EddyProc.C$sPlotFingerprint('Reco_DT_CUT_16', Dir = outdir)
...
for (Yr in c(2005:2023))
{
  ...
  a = ggplot(Year_Filled_Eddy_Data, aes(x = Time, y = Date, fill = NEE_CUT_16_f)) + geom_tile() + scale_fill_viridis(option = "D") +
    ggtitle('NEE') + theme(axis.text.x=element_blank())
  
  b = ggplot(Year_Filled_Eddy_Data, aes(x = Time, y = Date, fill = GPP_CUT_16_f)) + geom_tile() + scale_fill_viridis(option = "D") +
    ggtitle('GPP (nighttime partitioning)') + theme(axis.text.x=element_blank())
  
  c = ggplot(Year_Filled_Eddy_Data, aes(x = Time, y = Date, fill = Reco_CUT_16)) + geom_tile() + scale_fill_viridis(option = "D") +
    ggtitle('RECO (nighttime partitioning)') + theme(axis.text.x=element_blank())  
  
  d = ggplot(Year_Filled_Eddy_Data, aes(x = Time, y = Date, fill = GPP_DT_CUT_16)) + geom_tile() + scale_fill_viridis(option = "D") +
    ggtitle('GPP (daytime partitioning)') + theme(axis.text.x=element_blank())
  
  e = ggplot(Year_Filled_Eddy_Data, aes(x = Time, y = Date, fill = Reco_DT_CUT_16)) + geom_tile() + scale_fill_viridis(option = "D") +
    ggtitle('RECO (daytime partitioning)') + theme(axis.text.x=element_blank())    
  
  gr = gridExtra::arrangeGrob(a,b,c,d,e, ncol = 5)
  
  ggsave(paste(outdir, as.character(Yr),"_CH-CHA_CUT16_NEE_GPP_RECO_", run_id,".png",sep=""), gr ,height = 200, width = 450, units = 'mm')
...
FilledEddyData.F_daily = FilledEddyData.F %>% group_by(Date) %>% 
  summarise(NEE_CUT_16_f_mean = mean(NEE_CUT_16_f),
            GPP_CUT_16_f_mean = mean(GPP_CUT_16_f),
            Reco_CUT_16_mean = mean(Reco_CUT_16),
            GPP_DT_CUT_16_mean = mean(GPP_DT_CUT_16),
            Reco_DT_CUT_16_mean = mean(Reco_DT_CUT_16),
...
a = ggplot(FilledEddyData.F_daily, aes(x = DOY, y = Year, fill = NEE_CUT_16_f_mean)) + geom_tile() + scale_fill_viridis(option = "D") +
  ggtitle('NEE_f_mean daily gapfilled') + theme(axis.text.x=element_blank()) + theme_bw()

b = ggplot(FilledEddyData.F_daily, aes(x = DOY, y = Year, fill = GPP_CUT_16_f_mean)) + geom_tile() + scale_fill_viridis(option = "D") +
  ggtitle('GPP_f_mean daily gapfilled') + theme(axis.text.x=element_blank()) + theme_bw()

c = ggplot(FilledEddyData.F_daily, aes(x = DOY, y = Year, fill = Reco_CUT_16_mean)) + geom_tile() + scale_fill_viridis(option = "D") +
  ggtitle('RECO_mean daily gapfilled') + theme(axis.text.x=element_blank()) + theme_bw()

d = ggplot(FilledEddyData.F_daily, aes(x = DOY, y = Year, fill = GPP_DT_CUT_16_mean)) + geom_tile() + scale_fill_viridis(option = "D") +
  ggtitle('GPP_DT_mean daily gapfilled') + theme(axis.text.x=element_blank()) + theme_bw()

e = ggplot(FilledEddyData.F_daily, aes(x = DOY, y = Year, fill = Reco_DT_CUT_16_mean)) + geom_tile() + scale_fill_viridis(option = "D") +
...
gr = gridExtra::arrangeGrob(a,b,c,d,e,f,g,h,i,j, ncol = 4)
